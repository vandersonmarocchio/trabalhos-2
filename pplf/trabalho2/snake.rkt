...
(define atualiza-cobra (lambda ()
  ...
  (draw-block dc (list-ref pepsi 0) (list-ref pepsi 1) "red") ; desenha a pepsi
...
(define lost-the-gaem (lambda ()
  (send dc draw-text "Fim de jogo" (- (round (/ (* BLOCKS_WIDTH BLOCK_SIZE) 2)) 75) (- (round (/ (* BLOCKS_HEIGHT BLOCK_SIZE) 2)) 60))
  (send dc draw-text "Pontos:" (- (round (/ (* BLOCKS_WIDTH BLOCK_SIZE) 2)) 75) (- (round (/ (* BLOCKS_HEIGHT BLOCK_SIZE) 2)) 40))
  (send dc draw-text (number->string pontos) (- (round (/ (* BLOCKS_WIDTH BLOCK_SIZE) 2)) -10) (- (round (/ (* BLOCKS_HEIGHT BLOCK_SIZE) 2)) 40))
  (send dc draw-text "Recorde:" (- (round (/ (* BLOCKS_WIDTH BLOCK_SIZE) 2)) 75) (- (round (/ (* BLOCKS_HEIGHT BLOCK_SIZE) 2)) 20))
  (send dc draw-text (number->string recorde) (- (round (/ (* BLOCKS_WIDTH BLOCK_SIZE) 2)) -20) (- (round (/ (* BLOCKS_HEIGHT BLOCK_SIZE) 2)) 20))                      
  (send dc draw-text "(r) recomeçar" (- (round (/ (* BLOCKS_WIDTH BLOCK_SIZE) 2)) 75) (- (round (/ (* BLOCKS_HEIGHT BLOCK_SIZE) 2)) 0))                     
))
...
(define (andando-sozinho)
  (define x  (- (pega-cabeca 0 cobra) (list-ref pepsi 0)))
  (define y  (- (pega-cabeca 1 cobra) (list-ref pepsi 1)))
  (define abs-x (abs x))
  (define abs-y (abs y))
  (cond
   [(> abs-x abs-y)
    (if (> x 0) (if (eq? direcao 'r) 'u 'l)
                (if (eq? direcao 'l) 'd 'r))]
   [else
    (if (> y 0) (if (eq? direcao 'd) 'l 'u)
                (if (eq? direcao 'u) 'r 'd))]))