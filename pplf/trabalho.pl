...
inicio:-
	new(Menu, dialog('Trabalho de PPLF', size(1000,1000))),
	new(L,label(nombre,'Sistema de Diagnóstico Médico')),
	new(A,label(nombre,'Tentando entender essa merda!!')),
	new(@texto,label(nombre,'Responde um questionário ai')),
	new(@respl,label(nombre,'')),
	new(Salir,button('Sair',and(message(Menu, destroy),message(Menu,free)))),
	new(@boton,button('Realizar Teste',message(@prolog,botones))),


	send(Menu,append(L)),new(@btncarrera,button('Diagnóstico')),
	send(Menu,display,L,point(35,20)),
	send(Menu,display,A,point(35,360)),
	send(Menu,display,@boton,point(35,150)),
	send(Menu,display,@texto,point(35,100)),
